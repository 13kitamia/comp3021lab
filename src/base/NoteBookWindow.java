...
public class NoteBookWindow extends Application {
	...
	private HBox addHBox() {
		...
		textSearch.setPrefSize(150, 20);
		Button buttonSearch = new Button("Search");
		buttonSearch.setPrefSize(75, 20);
	...
	private VBox addVBox() {
		...
		for(Folder folder : noteBook.getFolders())
			foldersComboBox.getItems().add(noteBook.getFolders().get(noteBook.getFolders().indexOf(folder)).getName());
		...
		titleslistView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Object>() {
			...
			public void changed(ObservableValue ov, Object t, Object t1) {
				...
				// textAreaNote
				String content = "";
				Folder current = noteBook.getFolders().get(0);
				for(Folder folder : noteBook.getFolders()){
					if(folder.getName().equals(currentFolder))
						current = folder;
				}
				for(Note note : current.getNotes()){
					if(note instanceof TextNote && note.getTitle().equals(title)){
						TextNote currentNote = (TextNote) note;
	...
	private void updateListView() {
		...
		if(currentSearch.isEmpty()){
			...
			for(Note note : current.searchNotes(currentSearch))
				list.add(note.getTitle());
		}
